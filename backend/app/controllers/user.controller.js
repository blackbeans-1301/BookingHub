...
exports.login = (req, res, next) => {
    ...
    }).catch(err => {res.status(500).send({Message: ("Some errors occurred on the server-side: " + err.message) || "Some errors occurred on the server-side "})})
}
...
exports.authenticationJWT = (req, res, next) => {
    ...
    if (!token) return res.status(400).send({Message: "Can't authentication user (no token part)"})
    try {
        // xac thuc token sync
